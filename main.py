...
# funzione per splittare una lista in sottoliste di pari lunghezza pari a 'n' elementi (crea una lista di liste). serve per il decritta e reincritta 
def split_lista(lista_input,n):
    newlist = [lista_input[i:i+n] for i in range(0, len(lista_input), n)]
    return newlist
...
class MasterPasswordPage(Screen):
    ...
    def imposta_nuova_mp(self):
        ...
        if master_password == conferma_mp and master_password != '':
            ...
            try:
                ...
                conferma_mp = Popup(title='Avviso', content=Label(text='Master Password creata \ncon successo'), size_hint=(0.8, 0.4))
                conferma_mp.open()
    ...
    def decritta_reincritta(self, mp_nuova_encoded):
        ...
        # numero degli item per ciascuna chiave in rubirca (ossia numero dei dati criptati per ogni chiave in rubrica)
        numero_item = 4

        nuovi_elementi_cifrati = split_lista(nuovi_elementi_cifrati, numero_item)
        print(nuovi_elementi_cifrati)
        
        # riaggrego tutte le sottoliste in una unica lista
        lista_normale = []

        for item in nuovi_elementi_cifrati:
            lista_normale.append(item)
