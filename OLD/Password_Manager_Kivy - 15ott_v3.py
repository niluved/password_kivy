...
class ChoicePage2(Screen):
    ...
    def reveal_aggiorna(self,scope):
        ...
        # verifico se è stato passato l'argomento 'reveal' per rivelare i dati criptati e passare alla pagina 'vista_Dati', se 'scope' è 'aggiorna' invece la funzione è stata lanciata dal pulsante per aggiornare i dati dell'account e quindi popolo tutti i campi dei textinput con i valori decriptati per essere aggiornati dall'utente
        if scope == 'reveal':
    ...
    def rimuovi_account(self):
        ...
        print(f"\nElemento '{site}' rimosso con successo.\n")
        
        # crea un popup per conferma la riuscita dell'operazione
        conferma_eliminazione = Popup(title='Avviso', content=Label(text='Rimozione dati avvenuta con successo'), size_hint=(0.4, 0.4))
...
class InserimentoDati(Screen):
    ...
    def aggiungi_password(self):
        ...
        scrivi_file(contents)                   # scrive il nuovo contenuto nel file json
        print('\nTutti i valori sono stati criptati con successo.\n')
...
class ListaRubrica(RecycleView):
    ...
    def printa_account(self, i):
        global selezione # imposto la 'selezione' come variabile globale così non devo richiamare la funzione da capo per riaverla (avrei il problema di passargli il parametro di nuovo)
...
# todo: inserire una progressbar/popup nella funzione aggiungipassword in InserimentoDati
# todo: aggiungere la funzione in Vistadati per copiare nella clipboard i dati decrittati 
